...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        train_c = x.size(0)
        out = torch.mm(x.view(train_c, -1), w) + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        dw = torch.mm(x.view(train_c, -1).t()
        , dout)
        db = torch.mm(dout.t(), torch.ones((train_c, 1)
        , dtype=dx.dtype, device = 'cuda')).squeeze(1)
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        dout, fc_cache = Linear.forward(x, w, b)
        out, relu_cache = ReLU.forward(dout)
    ...
    def backward(dout, cache):
        ...
        dx, dw, db =  Linear.backward(dout, fc_cache)
...
def softmax_loss(x, y):
    ...
    shifted_x = x - torch.max(x, dim=1, keepdim=True).values
    
    exp_x = torch.exp(shifted_x)
       
    probabilities = exp_x / torch.sum(exp_x, dim=1, keepdim=True)
    N = x.shape[0]  # Number of examples
    loss = -torch.log(probabilities[range(N), y]).mean()
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_linear = Linear.forward(out, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i_layer in range(self.num_layers):
            ...
            size = (layers_dims[i_layer], layers_dims[i_layer+1])).to(device)
    ...
    def loss(self, X, y=None):
        ...
        for i_layer in range(self.num_layers):
            if i_layer == self.num_layers - 1:
                scores, cache = Linear.forward(scores, self.params[f'W{i_layer+1}'], 
                self.params[f'b{i_layer+1}'])
            else:
                scores, cache = Linear_ReLU.forward(scores, self.params[f'W{i_layer+1}'],
                 self.params[f'b{i_layer+1}'])
            caches.append(cache)
        #################################################################
        #                      END OF YOUR CODE                         #
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - learning_rate * dw / (torch.sqrt(config['cache']) + epsilon)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            # Scale the output during training to account for dropped elements
            out = x * mask / (1 - p)
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
